...
import { CdnResultDTO, ServiceResultDTO } from "../dto/result.dto";
...
import { transactionConvertFormData, updateTransactionConvertFormData } from "../utils/convert.form-data";
...
export class TransactionController {
  async createTransaction(req: Request, res: Response): Promise<void> {
    ...
    try {
      const result : ServiceResultDTO = await transactionService.createTransaction(transactionInput);
  ...
  async getTransaction(req: Request, res: Response): Promise<void> {
    ...
    try {
      const result : ServiceResultDTO = await transactionService.getTransaction(transaction_id);
      res.status(200).json(result);
    } catch (error) {
      if (error instanceof CustomError) {
        res.status(error.statusCode).json({ message: error.message });
  ...
  async updateTransaction (req: Request, res: Response): Promise<void> {
    ...
    //   }
      ...
      try {
        const result: ServiceResultDTO = await transactionService.updateTransaction(transaction_id, transactionInput);
        res.status(200).json(result);
      } catch (error: any) {
        if (error instanceof CustomError) {
          res.status(error.statusCode).json({ message: error.message });
...
async deleteTransaction (req: Request, res: Response): Promise<void> {
    ...
    try {
      const result: ServiceResultDTO = await transactionService.deleteTransaction(transaction_id);
      res.status(200).json(result);
    } catch (error: any) {
      console.log(error);
      if (error instanceof CustomError) {
